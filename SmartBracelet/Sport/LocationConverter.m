...
+ (NSData *)sendOTAPackData:(NSData *)data packIndex:(NSUInteger)packIndex {
    NSUInteger length = data.length;
    uint8_t *tempData=(uint8_t *)[data bytes];
    uint8_t pack_head[2];
    pack_head[1] = (packIndex >>8)& 0xff;
    pack_head[0] = (packIndex)&0xff;
    ...
    //CRC
    unsigned short crc_t = crc16222(otaCmd, (int)length+2);
    uint8_t crc[2];
    crc[1] = (crc_t >> 8) & 0xff;
    crc[0] = (crc_t)&0xff;
...
// 计算CRC16校验值的函数
unsigned short crc16222(unsigned char *pD, int len) {
