...
typedef void(^DialCreateFile_BK)(float progress);
/// 新增表盘(文件)
/// @param path “/文件名”
/// @param data 文件数据内容
/// @param result 进度
+(BOOL)makeCreateFile:(NSString*)path
              Content:(NSData* __nullable)data
               Result:(DialCreateFile_BK __nullable)result;

typedef void(^DialReplaceFile_BK)(float progress);
/// 替换表盘(文件)
/// @param path “/文件名”
/// @param data 文件数据内容
+(BOOL)makeReplaceFile:(NSString*)path
               Content:(NSData* __nullable)data
                Result:(DialReplaceFile_BK __nullable)result;
