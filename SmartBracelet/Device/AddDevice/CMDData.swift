...
class CMDData: NSObject {
    ...
    private func recordBaseTimeNotify(data: Data) {
        ...
        XLogger.shared.log("手环返回的时间：\(2000 + year)-\(month)-\(day) \(hour):\(minute):\(second)")
    }
    
    private func recordsNotify(data: Data) {
        let seq = Int(data[2]) + Int(data[3]) * Int(256)
        XLogger.shared.log("当前包序号：\(seq)")
        let num = Int(data[4])
        XLogger.shared.log("How much records in this Seq, range \(num)")
        if num >= 1 {
            ...
            XLogger.shared.log("record1: \(record1) record1Minutes: \(record1Minutes) record1Step: \(record1Step)")
            if record1Step > 0 {
                ...
                do {
                    ...
                    XLogger.shared.log("步数保存成功")
                } catch {
                    XLogger.shared.log("步数保存失败：\(error.localizedDescription)")
                }
        ...
        if num >= 2 {
            ...
            XLogger.shared.log("record2: \(record2) record2Minutes: \(record2Minutes) record2Step: \(record2Step)")
            if record2Step > 0  {
                let stepModel = DStepModel()
                stepModel.mac = bleSelf.bleModel.mac
                stepModel.uuidString = bleSelf.bleModel.uuidString
        ...
        if num > 2 {
            ...
            XLogger.shared.log("record3: \(record3) record3Minutes: \(record3Minutes) record3Step: \(record3Step)")
            if record3Step > 0  {
                let stepModel = DStepModel()
                stepModel.mac = bleSelf.bleModel.mac
                stepModel.uuidString = bleSelf.bleModel.uuidString
    ...
    private func currentStateNotify(data: Data) {
        ...
        XLogger.shared.log("record: \(record1) recordMinutes: \(record1Minutes) recordStep: \(record1Step)")
        let year = Int(data[7])
        ...
        XLogger.shared.log("手环返回的currnet state时间：\(2000 + year)-\(month)-\(day) \(hour):\(minute):\(second)")
    ...
    private func recordBaseTimeForHRBPNotify(data: Data) {
        ...
        let minute = Int(data[6])
        let second = Int(data[7])
        XLogger.shared.log("手环返回的record时间：\(2000 + year)-\(month)-\(day) \(hour):\(minute):\(second)")
    }
    ...
    private func recordsForHRBPNotify(data: Data) {
        ...
        if num >= 1 {
            ...
            XLogger.shared.log("seconds1: \(seconds1) value1: \(value1)")
        }
        if num >= 2 {
            ...
            XLogger.shared.log("seconds2: \(seconds2) value2: \(value2)")
        }
    }
    
    private func endSyncForHRBPNotify(data: Data) {
        XLogger.shared.log("end sync")
    }
    ...
    public func handleDeviceNameNotify(data: Data) {
        ...
        XLogger.shared.log("版本号：\(firmwareVersion) deviceCapability: \(deviceCapability)")
    }
    ...
    public func handleDeviceBatteryNotify(data: Data) {
        ...
        XLogger.shared.log("battery: \(battery) battery state:\(batteryState)")
        XLogger.shared.log("手环时间：\(2000 + year)-\(month)-\(day) \(hour):\(minute):\(second)")
    ...
    private func flagLess20Notify(flag: Int, data: Data) {
        ...
        XLogger.shared.log("state: \(state)")
        let stepCount = Int(data[3]) + Int(data[4]) * 256 + Int(data[5]) * 256 * 256
        XLogger.shared.log("stepCount: \(stepCount)")
        let longsit = Int(data[7])
        XLogger.shared.log("longsit: \(longsit)")
        let shake = Int(data[8])
        XLogger.shared.log("shake: \(shake)")
        let sensorState = Int(data[9])
        XLogger.shared.log("sensorState: \(sensorState)")
        let heartRate = Int(data[10])
        XLogger.shared.log("heartRate: \(heartRate)")
        let battery = Int(data[11])
        let batteryState = Int(data[12])
        XLogger.shared.log("设备电量：\(battery) 设备电量状态：\(batteryState)")
        if flag == 0x10 {
            let subData = Data(data[13...16])
            let firmwareVersion = String(data: subData, encoding: .utf8) ?? ""
            XLogger.shared.log("设备当前版本号：\(firmwareVersion)")
        }
        let hrbpState = Int(data[17])
        XLogger.shared.log("hrbpState: \(hrbpState)")
    }
    ...
    private func flagLess100Notify(data: Data) { // Request phone to sync records
        ...
        XLogger.shared.log("length: \(length) data[3]: \(data[3]) data[4]:\(data[4])")
    }
    ...
    private func flagLess40Notify(data: Data) { // Step raw data
        ...
        XLogger.shared.log("count: \(count) stepCount: \(stepCount)")
    }
    ...
    private func syncStartNotify(data: Data) {
        ...
        let minute = Int(data[6])
        let second = Int(data[7])
        XLogger.shared.log("sync data：\(2000 + year)-\(month)-\(day) \(hour):\(minute):\(second)")
    }
    ...
    private func syncRecordNotify(data: Data) {
        ...
        XLogger.shared.log("state: \(state) stepCount: \(stepCount) minutes: \(minutes)")
    }
    ...
    private func syncCurrentStateNotify(data: Data) {
        ...
        XLogger.shared.log("state: \(state) stepCount: \(stepCount) minutes: \(minutes)")
        XLogger.shared.log("current state：\(2000 + year)-\(month)-\(day) \(hour):\(minute):\(second)")
    }
    
    private func syncEndNotify(data: Data) {
        XLogger.shared.log("sync end")
    }
