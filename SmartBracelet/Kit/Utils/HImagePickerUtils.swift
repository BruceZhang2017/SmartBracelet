...
class HImagePickerUtils: NSObject,UIImagePickerControllerDelegate,UINavigationControllerDelegate {
    ...
    static func showTips(at rootVC:UIViewController!,type:HChooseType) {
        if #available(iOS 8.0, *) {
            let alertVC = UIAlertController(title: nil, message: "您阻止了\(type.description())访问权限", preferredStyle: .alert)
            let openIt = UIAlertAction(title: "马上打开", style: .default, handler: { (alert:UIAlertAction) -> Void in
            ...
            let alertVC = UIAlertController(title: "hint".localized(), message: "请在 '系统设置|隐私|\(type.description())' 中开启相机访问权限", preferredStyle: .alert)
            rootVC.present(alertVC, animated: true, completion: nil)
        }
    }
}
