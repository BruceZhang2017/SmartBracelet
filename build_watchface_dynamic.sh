...
# WatchFaceSDK-ObjC 动态库 Framework 构建脚本
# 使用 Output-ObjC-Dynamic 中的 WatchProtocolSDK 动态库
...
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  WatchFaceSDK-ObjC 动态库构建${NC}"
echo -e "${BLUE}  使用 WatchProtocolSDK 动态库${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
...
BUILD_DIR="$PROJECT_DIR/build/WatchFaceObjC-Dynamic"
OUTPUT_DIR="$PROJECT_DIR/Output-WatchFace-ObjC"

# 使用动态库版本的 WatchProtocolSDK
...
if [ ! -d "$WATCHPROTOCOL_FRAMEWORK" ]; then
    echo -e "${RED}❌ WatchProtocolSDK.xcframework 不存在于 Output-ObjC-Dynamic${NC}"
    echo -e "   请先运行: ./build_watchprotocol_objc_dynamic.sh"
    exit 1
fi
echo -e "   ✅ WatchProtocolSDK.xcframework (动态库)"

# 验证是动态库
DEVICE_LIB="$WATCHPROTOCOL_FRAMEWORK/ios-arm64/WatchProtocolSDK.framework/WatchProtocolSDK"
if [ -f "$DEVICE_LIB" ]; then
    LIB_TYPE=$(file "$DEVICE_LIB" | grep -o "dynamically linked shared library")
    if [ -z "$LIB_TYPE" ]; then
        echo -e "${RED}❌ 错误: WatchProtocolSDK 不是动态库${NC}"
        exit 1
    fi
    echo -e "   ✅ 已验证为动态库"
fi
...
# 创建动态库 - 链接到动态版本的 WatchProtocolSDK
echo -e "${GREEN}🔗 链接 Device Framework (动态链接)...${NC}"
...
xcrun clang -dynamiclib \
    ...
    -F"$WATCHPROTOCOL_FRAMEWORK/ios-arm64" \
...
# 创建 Simulator Framework (动态链接)
echo -e "${GREEN}🔗 链接 Simulator Framework (动态链接)...${NC}"
...
xcrun clang -dynamiclib \
    ...
    -framework CoreGraphics \
    -framework CoreBluetooth \
    -F"$WATCHPROTOCOL_FRAMEWORK/ios-arm64_x86_64-simulator" \
    -framework WatchProtocolSDK \
...
# 验证动态库链接
echo ""
echo -e "${GREEN}🔍 验证动态库依赖...${NC}"
echo "Device (arm64):"
otool -L "$OUTPUT_DIR/${FRAMEWORK_NAME}.xcframework/ios-arm64/${FRAMEWORK_NAME}.framework/${FRAMEWORK_NAME}" | grep -E "WatchProtocolSDK|ABParTool"

echo ""
echo "Simulator (arm64/x86_64):"
otool -L "$OUTPUT_DIR/${FRAMEWORK_NAME}.xcframework/ios-arm64_x86_64-simulator/${FRAMEWORK_NAME}.framework/${FRAMEWORK_NAME}" | grep -E "WatchProtocolSDK|ABParTool"

echo ""
echo -e "${GREEN}🎉 动态库 Framework 构建成功！${NC}"
echo -e "${BLUE}注意: 使用此 framework 时，需要同时包含 WatchProtocolSDK 和 ABParTool 动态库${NC}"
